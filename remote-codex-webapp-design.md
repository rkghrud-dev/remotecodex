...
컴퓨터를 잘 다루지 못하는 지인의 PC에 작은 프로그램을 설치해 두고, 내 PC의 Host 앱에서 그 PC에 접속하여 스크린샷 확인, 클릭/키보드 입력, PowerShell, Codex CLI, 파일 전송, Agent AI 작업 지시를 통해 원격으로 개발 지원을 할 수 있게 만든다.

핵심 목표는 TeamViewer 같은 완전한 원격 데스크톱을 처음부터 복제하는 것이 아니라, 코딩 지원에 필요한 조작을 더 쉽게 수행하는 하이브리드 원격 지원 앱을 만드는 것이다.
...
웹브라우저만으로는 상대 컴퓨터의 화면 캡처, 마우스/키보드 입력, PowerShell 실행, 로컬 파일 수정 권한을 안정적으로 줄 수 없으므로, 상대 PC에는 Windows용 Guest Agent 프로그램을 설치한다.

이 앱의 원격 조작 방식은 아래 3단계로 발전시킨다.

```text
1단계: 스크린샷 + 클릭/키입력 + PowerShell + 파일 전송
2단계: Agent AI/Codex가 상대 PC에서 작업 수행
3단계: 낮은 FPS 화면 스트리밍과 드래그/스크롤 확장
```
...
Guest Agent에서 우선 사용할 Windows 기능:

- 화면 캡처: Windows Graphics Capture 또는 GDI 캡처
- 마우스/키보드 입력: SendInput
- 창 정보 조회: Win32 API
- PowerShell 실행: System.Diagnostics.Process
- Codex 실행: PowerShell 또는 Process로 codex CLI 호출
- 파일 처리: .NET File/Directory API
...
- 스크린샷 위 클릭 좌표 전송
- 키보드 텍스트 입력 전송
- Enter, Esc, Tab, Ctrl+C 같은 특수키 전송
- 활성 창 제목 표시
- 다시 캡처 버튼
- 입력 중단 버튼

이 화면은 실시간 원격 데스크톱이 아니라, 필요할 때 스크린샷을 갱신하고 클릭/키보드 입력을 보내는 반자동 조작 화면이다.

### 6.4 AI 작업 지시 화면

기능:

- 상대 PC에서 수행할 작업 설명 입력
- 현재 스크린샷 첨부
- 선택한 작업 폴더 첨부
- PowerShell 실행 승인
- Codex CLI 실행 승인
...
```text
크롬을 열고 이 사이트 로그인 오류를 확인한 뒤, 필요한 설정을 수정해줘.
```

```text
바탕화면 project 폴더에서 Codex를 실행해서 빌드 오류를 고쳐줘.
```
...
이 프로젝트는 완전한 원격 데스크톱보다 하이브리드 조작을 우선한다.

### 7.1 직접 조작

Host에서 스크린샷을 보고 필요한 입력만 보낸다.

```text
1. Host에서 스크린샷 요청
2. Guest가 화면 캡처 후 이미지 전송
3. Host에서 이미지 위 특정 위치 클릭
4. Guest가 해당 좌표에 마우스 클릭 실행
5. Host에서 키보드 입력 전송
6. Guest가 활성 창에 키보드 입력 실행
7. Host에서 다시 스크린샷 요청
```

장점:

- 실시간 화면 스트리밍보다 구현이 쉽다.
- 네트워크 딜레이에 덜 민감하다.
- 코딩 지원, 설치 지원, 설정 변경에 충분히 쓸 수 있다.

한계:

- 빠른 드래그나 게임처럼 즉각적인 조작에는 맞지 않는다.
- 화면이 바뀔 때마다 스크린샷을 다시 요청해야 한다.

### 7.2 Agent AI 조작

Host에서 작업을 자연어로 지시하면 Guest가 PowerShell, Codex CLI, 파일 조작, 브라우저 자동화 등을 조합해서 수행한다.

```text
1. Host에서 작업 지시 입력
2. Host가 현재 스크린샷/작업 폴더/사용자 승인 옵션을 함께 전달
3. Guest가 필요한 명령을 계획
4. 위험하거나 큰 변경 전 Host에 승인 요청
5. Guest가 PowerShell, Codex CLI, 파일 수정, 클릭/입력을 실행
6. 결과 로그와 스크린샷을 Host에 보고
```

Agent AI 조작은 원격 데스크톱을 대체하는 보조 방식이다. 사용자는 매번 마우스로 조작하지 않고, "이 문제 확인해줘", "Codex로 오류 고쳐줘" 같은 작업 단위로 지시할 수 있다.
...
- 클릭/키보드 입력 로그 저장
- AI Agent가 파일 수정 또는 명령 실행 전 승인받는 모드 제공
...
MVP에서는 음성 기능을 제외하고, 스크린샷 조작, 터미널, 파일 전송을 먼저 완성한다.
...
8. Host에서 스크린샷 보기 구현
9. Host에서 클릭 좌표 보내기
10. Guest에서 마우스 클릭 실행
11. Host에서 키보드 입력 보내기
12. Guest에서 키보드 입력 실행
13. Host에서 PowerShell 명령 보내기
14. Guest에서 PowerShell 실행
15. 출력 실시간 표시
16. 파일 업로드/다운로드 구현
17. Codex CLI 실행 편의 기능 추가
18. Agent AI 작업 지시 화면 추가
19. Host/Guest 설치 파일 제작
20. 낮은 FPS 화면 스트리밍과 음성 기능 검토
...
MVP 기능: 스크린샷 확인 + 클릭/키입력 + PowerShell 원격 실행 + 파일 전송
```
